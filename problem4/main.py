...
# Our lives would be easier without the linear time constraint: we would just sort the array, while filtering out negative numbers, and iterate over the sorted array and return the first number that doesn't match the index. 
...
def first_missing_positive(nums):
    ...
    for i, num in enumerate(nums):
        while i + 1 != nums[i] and 0 < nums[i] <= len(nums):    # When we have a number to swap
            v = nums[i]
    ...
    for i, num in enumerate(nums):
        if num != i:
            return i
    return len(nums) + 1    # if all matcehs to pattern
